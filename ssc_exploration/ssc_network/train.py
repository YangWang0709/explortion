    ...
    for epoch in range(0, args.epochs):
        ...
        for _, (rgb, depth, _, target, position, _) in tqdm(enumerate(train_loader), desc=decs_str, unit='step'):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError('Non-finite training loss at epoch {}'.format(epoch + 1))
        ...
        avg_train_loss = running_loss / train_step_count if train_step_count else float('nan')
        print('Train:epoch {}, loss {:.6f}'.format(epoch + 1, avg_train_loss))

        scheduler.step()

        # ---- Evaluate on validation set
        v_prec, v_recall, v_iou, v_acc, v_ssc_iou, v_mean_iou, _ = validate_on_dataset(
            net, val_loader)
