...
try:
    ...
    def precision_recall_fscore_support(y_true, y_pred, average='binary'):
        ...
        fn = np.logical_and(y_true == 1, y_pred != 1).sum()
        precision = float(tp / (tp + fp)) if (tp + fp) else 0.0
        recall = float(tp / (tp + fn)) if (tp + fn) else 0.0
        fscore = float(2 * precision * recall / (precision + recall)) if (precision + recall) else 0.0
        return precision, recall, fscore, None
