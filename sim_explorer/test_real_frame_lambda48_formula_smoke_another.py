#!/usr/bin/env python3
"""Validate Stage 4A-6.5ae real Frame1 lambda48 formula smoke outputs."""
...
def test_content(output_dir: Path, expected_min_seeds: int) -> dict[str, Any]:
    frame_report = load_json(output_dir / "selected_frame_report.json")
    ...
    final_summary = load_json(output_dir / "stage4a65ae_real_frame1_lambda48_formula_summary.json")

    assert frame_report["selected_frame_index"] == 1
    assert frame_report["fallback_used"] is False
    assert frame_report["selected_stage4a65ad_frame2"] is False
    assert frame_report["no_silent_fallback_to_frame2"] is True

    assert manifest["stage"] == "Stage 4A-6.5ae"
    assert manifest["selected_frame_index"] == 1
    assert Path(manifest["observed_state"]["path"]).name == "observed_state_frame001.npy"
    assert "frame001_prediction" in manifest["prediction_npz"]["path"]
    assert Path(manifest["pose_json"]["path"]).name == "frame001_pose.json"
    assert Path(manifest["camera_info_json"]["path"]).name == "frame001_camera_info.json"
    assert manifest["prediction_npz"]["shape_aligned_to_observed_state"] is True
    assert manifest["tree_root_source"] == "rebuilt_from_frame001_pose_json"
    ...
    assert formula["stage"] == "Stage 4A-6.5ae"
    ...
    assert refs["measured_reference"]["best_descendant_id"] == "n0249"
    ...
    assert len(decisions) >= expected_min_seeds * 6, "not enough decision rows"
    ...
    assert lambda_summary["stage"] == "Stage 4A-6.5ae"
    assert lambda_summary["selected_frame"] == "Stage 4A-6.5p Frame1"
    ...
    assert comparison["selected_frame"] == "Stage 4A-6.5p Frame1"
    ...
    assert answers["loaded_stage4a65p_frame1_inputs"] is True
    assert answers["selected_frame_index"] == 1
    assert answers["no_isaac_no_capture_no_map_predict_rerun"] is True
    assert answers["seed_count"] >= expected_min_seeds
    assert answers["measured_only_reproduced_frame1_reference"] is True
