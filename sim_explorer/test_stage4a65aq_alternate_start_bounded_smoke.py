#!/usr/bin/env python3
"""Validate Stage 4A-6.5aq alternate-start bounded smoke outputs."""
...
def test_variant_and_hardware(output_dir: Path, args: argparse.Namespace, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    assert int(variant["reference_tree_seed_0"]) == int(args.expected_reference_tree_seed_0)
    assert int(variant["reference_tree_seed_1"]) == int(args.expected_reference_tree_seed_1)
    assert int(variant["reference_tree_seed_2"]) == int(args.expected_reference_tree_seed_2)
    assert variant["only_intended_runtime_change"] == "start_pose"
    assert variant["tree_seed_matches_reference_seed0"] is True
    assert variant["start_pose_changed_vs_canonical_references"] is True
    ...
    assert alternate["matches_stage4a65ap_design"] is True
...
def test_repeat_outputs(output_dir: Path, summary: dict[str, Any]) -> dict[str, Any]:
    design = load_json(output_dir / "comparison_to_stage4a65ap_design.json")
    combined = load_json(output_dir / "comparison_to_canonical_seed012.json")
    repeat = load_json(output_dir / "alternate_start_outcome_classification.json")
    lambda_repeat = load_json(output_dir / "lambda32_vs_lambda48_alternate_start.json")
    readiness = load_json(output_dir / "alternate_start_safety_readiness_matrix.json")
    reference = load_json(output_dir / "loaded_reference_manifest.json")

    assert reference["loaded_stage4a65ak_summary"] is True
    assert reference["loaded_stage4a65am_summary"] is True
    assert reference["loaded_stage4a65ao_summary"] is True
    assert reference["loaded_stage4a65ap_review_design_summary"] is True
    assert reference["loaded_alternate_start_metadata"] is True
    assert design["design_matched"] is True
    assert combined["canonical_exact_match_not_required_because_start_changed"] is True
    assert combined["current_tree_seed"] == summary["repeat_variant"]["current_tree_seed"]
    assert combined["comparison_to_stage4a65ak"]["frame001"]["available"] is True
    assert combined["comparison_to_stage4a65am"]["frame001"]["available"] is True
    assert combined["comparison_to_stage4a65ao"]["frame001"]["available"] is True
    assert "alternate_start_outcome" in repeat
    assert repeat["alternate_start_outcome"] in {
        "clean_same_as_measured",
        "clean_distinct_nonmeasured",
        "spatially_consistent_healthy_alternate_start",
        "start_sensitive_but_clean",
        "artifact_or_prior_basin_regression",
        "action_blocked",
        "runtime_failure",
    }
    if int(summary["runtime_setup"]["selected_action_execution_count"]) == 1:
        assert combined["comparison_to_stage4a65ak"]["frame002"]["available"] is True
        assert combined["comparison_to_stage4a65am"]["frame002"]["available"] is True
        assert combined["comparison_to_stage4a65ao"]["frame002"]["available"] is True
    ...
    return {"passed": True, "alternate_start_outcome": repeat["alternate_start_outcome"]}
...
def run_tests(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = load_json(output_dir / "stage4a65aq_alternate_start_summary.json")
    results = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage4a65ap_dir", required=False)
    parser.add_argument("--stage4a65ak_dir", required=True)
    parser.add_argument("--stage4a65am_dir", required=False)
    parser.add_argument("--stage4a65ao_dir", required=False)
    parser.add_argument("--expected_repeat_variant", default="alternate_start_corridor_tree_seed0")
    ...
    parser.add_argument("--expected_reference_tree_seed_0", type=int, default=0)
    parser.add_argument("--expected_reference_tree_seed_1", type=int, default=1)
    parser.add_argument("--expected_reference_tree_seed_2", type=int, default=2)
