#!/usr/bin/env python3
"""Run a small sequential Stage 4A-6.4 SC gain-gating ablation sweep."""
...
class GatingConfig:
    ...
    sc_count_mode: str = "selective"
    calibration_table: str | None = None


CONFIGS: dict[str, GatingConfig] = {
    "occupied_only_occ07": GatingConfig("occupied_only_occ07", "occupied_only", 0.7, 0.3, 1.0, None),
    "occupied_only_occ08": GatingConfig("occupied_only_occ08", "occupied_only", 0.8, 0.3, 1.0, None),
    "occupied_margin_occ06_w05": GatingConfig(
    ...
    "confidence_weighted_conf05_cap30": GatingConfig(
    ...
    "calibrated_occupied": GatingConfig("calibrated_occupied", "calibrated_occupied", 0.7, 0.3, 1.0, None),
}


def _split_configs(value: str, calibration_table: str | None) -> list[GatingConfig]:
    out: list[GatingConfig] = []
    for raw in str(value).split(","):
        name = raw.strip()
        if not name:
            continue
        if name not in CONFIGS:
            raise ValueError(f"Unknown config {name!r}. Known configs: {sorted(CONFIGS)}")
        cfg = CONFIGS[name]
        if cfg.sc_gain_formula == "calibrated_occupied":
            if not calibration_table:
                raise ValueError("calibrated_occupied config requires --calibration_table")
            cfg = GatingConfig(
                name=cfg.name,
                sc_gain_formula=cfg.sc_gain_formula,
                sc_occ_threshold=cfg.sc_occ_threshold,
                sc_conf_threshold=cfg.sc_conf_threshold,
                sc_gain_weight=cfg.sc_gain_weight,
                sc_gain_cap=cfg.sc_gain_cap,
                score_gain_mode=cfg.score_gain_mode,
                sc_count_mode=cfg.sc_count_mode,
                calibration_table=str(Path(calibration_table).resolve()),
            )
        if cfg.name not in {item.name for item in out}:
            out.append(cfg)
    if not out:
...
def _episode_id(args: argparse.Namespace, cfg: GatingConfig) -> str:
    return f"{args.scene_variant}_seed{args.scene_seed}_{args.start_variant}_{cfg.name}"


def _child_args(args: argparse.Namespace, app_args: list[str], cfg: GatingConfig) -> list[str]:
    ...
    child = [
        ...
        cfg.sc_gain_formula,
        "--sc_occ_threshold",
        str(cfg.sc_occ_threshold),
        "--sc_conf_threshold",
        str(cfg.sc_conf_threshold),
        "--sc_count_mode",
        cfg.sc_count_mode,
        "--sc_gain_weight",
    ...
    if cfg.calibration_table is not None:
        child.extend(["--calibration_table", str(Path(cfg.calibration_table).resolve())])
...
def _record(args: argparse.Namespace, cfg: GatingConfig, returncode: int, wall_time: float, log_path: Path) -> dict[str, Any]:
    output_dir = Path(args.output_dir).resolve()
    episode_id = _episode_id(args, cfg)
    summary = _load_summary(output_dir, episode_id)
    status = "ok" if returncode == 0 and summary and int(summary.get("steps_completed", 0)) >= 1 else "failed"
    record: dict[str, Any] = {
        ...
        "sc_gain_formula": cfg.sc_gain_formula,
        "sc_occ_threshold": float(cfg.sc_occ_threshold),
        "sc_conf_threshold": float(cfg.sc_conf_threshold),
...
def run_sweep(args: argparse.Namespace, app_args: list[str]) -> dict[str, Any]:
    ...
    configs = _split_configs(args.configs, args.calibration_table)
    save_json(
        output_dir / "ablation_configs.json",
        {
            "stage": "Stage 4A-6.4 SC gain gating ablation configs",
            ...
            "original_sc_episode_dir": str(Path(args.original_sc_episode_dir).resolve()) if args.original_sc_episode_dir else None,
    ...
    if manifest_jsonl.exists():
        records = load_jsonl(manifest_jsonl)
    existing = {(str(r.get("config_name")), str(r.get("episode_id"))) for r in records}
    for cfg in configs:
        print(f"[GATING] starting {cfg.name}")
        cmd = _child_args(args, app_args, cfg)
        ...
        try:
            ...
            returncode = 1
            log_path.write_text(f"runner failed before child completed: {type(exc).__name__}: {exc}\n", encoding="utf-8")
        wall_time = time.perf_counter() - start
        record = _record(args, cfg, returncode, wall_time, log_path)
        key = (str(record["config_name"]), str(record["episode_id"]))
        if key not in existing:
            records.append(record)
            append_jsonl(manifest_jsonl, record)
            existing.add(key)
        else:
            records = [record if (str(r.get("config_name")), str(r.get("episode_id"))) == key else r for r in records]
            with manifest_jsonl.open("w", encoding="utf-8") as handle:
                for item in records:
                    handle.write(json.dumps(item, sort_keys=True, allow_nan=False) + "\n")
        print(
            f"[GATING] {cfg.name} status={record['status']} steps={record.get('steps_completed')} "
    ...
    summary = {
        "stage": "Stage 4A-6.4 SC gain gating ablation sweep",
...
def parse_args(argv: list[str] | None = None) -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(description="Run sequential Stage 4A-6.4 SC gain-gating ablations.")
    ...
    parser.add_argument("--original_sc_episode_dir", default=None)
...
def main() -> None:
    ...
    print("Stage 4A-6.4 SC gain gating ablation sweep complete.")
