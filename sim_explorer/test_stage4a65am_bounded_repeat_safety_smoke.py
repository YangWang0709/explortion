#!/usr/bin/env python3
"""Validate Stage 4A-6.5am bounded repeat-safety smoke outputs."""
...
def test_repeat_outputs(output_dir: Path, summary: dict[str, Any]) -> dict[str, Any]:
    comparison = load_json(output_dir / "comparison_to_stage4a65ak.json")
    ...
    assert reference["loaded_stage4a65al_summary"] is True
    assert comparison["reference_stage"] == "Stage 4A-6.5ak"
    assert comparison["current_tree_seed"] == summary["repeat_variant"]["current_tree_seed"]
    assert comparison["reference_tree_seed"] == summary["repeat_variant"]["reference_tree_seed"]
...
def run_tests(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = load_json(output_dir / "stage4a65am_bounded_repeat_safety_summary.json")
    results = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected_repeat_variant", default="same_scene_start_tree_seed1")
