#!/usr/bin/env python3
"""Validate Stage 4A-6.5ao bounded repeat-safety smoke outputs."""
...
def test_variant_and_hardware(output_dir: Path, args: argparse.Namespace, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    assert variant["only_intended_runtime_change"] == "tree_seed"
...
def test_repeat_outputs(output_dir: Path, summary: dict[str, Any]) -> dict[str, Any]:
    comparison_seed0 = load_json(output_dir / "comparison_to_stage4a65ak.json")
    comparison_seed1 = load_json(output_dir / "comparison_to_stage4a65am.json")
    combined = load_json(output_dir / "comparison_to_seed0_seed1_combined.json")
    repeat = load_json(output_dir / "repeat_stability_classification.json")
    lambda_repeat = load_json(output_dir / "lambda32_vs_lambda48_repeat.json")
    ...
    assert reference["loaded_stage4a65an_review_design_summary"] is True
    assert comparison_seed0["reference_stage"] == "Stage 4A-6.5ak"
    assert comparison_seed1["reference_stage"] == "Stage 4A-6.5am"
    assert comparison_seed0["current_tree_seed"] == summary["repeat_variant"]["current_tree_seed"]
    assert comparison_seed1["current_tree_seed"] == summary["repeat_variant"]["current_tree_seed"]
    assert comparison_seed0["reference_tree_seed"] == summary["repeat_variant"]["reference_tree_seed_0"]
    assert comparison_seed1["reference_tree_seed"] == summary["repeat_variant"]["reference_tree_seed_1"]
    assert combined["current_tree_seed"] == summary["repeat_variant"]["current_tree_seed"]
    assert comparison_seed0["frame001"]["available"] is True
    assert comparison_seed1["frame001"]["available"] is True
    assert "repeat_outcome" in repeat
    assert repeat["repeat_outcome"] in {
        "exact_repeat_match_to_seed0",
        "exact_repeat_match_to_seed1",
        "spatially_consistent_healthy_repeat",
        "divergent_but_healthy",
        "seed_sensitive_but_clean",
        "artifact_or_prior_basin_regression",
        "action_blocked",
        "runtime_failure",
    }
    if int(summary["runtime_setup"]["selected_action_execution_count"]) == 1:
        assert comparison_seed0["frame002"]["available"] is True
        assert comparison_seed1["frame002"]["available"] is True
...
def run_tests(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = load_json(output_dir / "stage4a65ao_bounded_repeat_safety_summary.json")
    results = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage4a65an_dir", required=False)
    parser.add_argument("--expected_repeat_variant", default="same_scene_start_tree_seed2")
    parser.add_argument("--expected_tree_seed", type=int, default=2)
